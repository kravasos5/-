...
for item in href_list:
	...
	file_oldname = os.path.join(f'data/', f'{i}_.html')
	file_newname_newfile = os.path.join(f'data/', f'{item_title}_.html')
...
for title, href in all_news_dict.items():
	...
	if message is not None:
		...
		if is_innovation(FINDING_WORDS, message) is True or is_innovation_root(FINDING_ROOTS, FINDING_ENDINGS, message) is True:
	...
	count += 1
	print(f'#Итерация: {count}, {title} просмотрен...')
	iteration_count -= 1
